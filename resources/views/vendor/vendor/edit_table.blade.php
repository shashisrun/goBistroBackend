...
<section class="section">
    ...
    <div class="section-header">
        ...
        <div class="section-header-breadcrumb">
            @if(Auth::user()->load('roles')->roles->contains('title', 'admin'))
                ...
                <div class="breadcrumb-item">{{__('Edit a Table')}}</div>
            @endif
    ...
    <div class="section-body">
        ...
        <p class="section-lead">{{__('Edit Table')}}</p>
        <div class="card p-3">
            <div class="card-body">
                <form class="container-fuild" action="{{ route('table.update', ['table' => $table->id]) }}" method="post">
                    @csrf
                    @method('PATCH')
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="Table">{{__('Table name')}}<span class="text-danger">&nbsp;*</span></label>
                            <input type="text" name="name" class="form-control @error('name') is_invalide @enderror" placeholder="{{__('Enter Table Name')}}" value="{{$table->name}}" required="">
                            @error('name')
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" name="capacity" class="form-control @error('email') is_invalide @enderror" id="capacity" placeholder="{{__('Enter Table Capacity')}}" value="{{$table->capacity}}" required="">
                            @error('capacity')
                                <span class="custom_error" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    <div class="text-center mt-3">
                        <button class="btn btn-primary" type="submit">{{__('Update Table')}}</button>
                    </div>
                </form>
            </div>
        </div>
